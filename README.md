...
## Repository v1.16.0-rc1 の実装範囲

### v1.16.0-rc1 Claude UAT Result Registration

- Claude応答JSONをUATセッションへ一括登録
- 登録前にClaude Output Contractと元依頼の整合性を検証
- 正規化済み応答・検証レポート・SHA-256を自動保存
- outcomeと未解決事項だけを反映し、人による点数・所見は自動生成しない
- 不正応答、欠落応答、既存結果の上書きを安全に拒否

実行例:

```bash
python -m runtime.sims_writer_runtime.cli \
  --register-claude-uat-results ./uat-sessions/UAT-ID \
  --uat-results-dir ./claude-results \
  --output ./uat-registration
```
...
### v1.16.0-rc1 UAT Evidence Ingestion

- prepared UAT session evidence ingestion
- request and response SHA-256 verification
- request/article identity consistency checks
- validation artifact presence checks
- tamper and missing-file detection
- session manifest status consolidation

```bash
python -m runtime.sims_writer_runtime.cli \
  --ingest-claude-uat-session ./uat-sessions/UAT-SESSION-ID \
  --output ./uat-report
```
...
## Claude実記事UATセッションの準備

実在記事の改善依頼JSONをフォルダへ配置し、追跡可能なUATセッションを作成します。

```bash
python -m runtime.sims_writer_runtime.cli \
  --prepare-claude-uat-session ./real-article-requests \
  --uat-session-id UAT-20260717-001 \
  --output ./uat-sessions
```

生成時点の証拠は`pending`かつ未採点です。入力依頼のSHA-256を保存し、Claude応答、検証結果、評価証拠を記事単位で対応付けます。
...
### v1.16.0-rc1 Claude Developer Rehearsal
