...
def test_version_and_contract():
 assert read("VERSION").strip()=="2.0.0-rc.4"
 s=json.loads(read("schemas/SIMS_FEEDBACK_V2.schema.json"))
 assert s["properties"]["contract_version"]["const"]=="4.1"
...
def test_no_competitor_copy_rule():
 ...
 assert "競合にあるから" in t and "推測値を作らない" in t
def test_claude_contract41():
 t=read("claude/CLAUDE_PROJECT_INSTRUCTIONS.md")
 assert "Contract 4.1" in t and "publication_result.serp_gap_report" in t
