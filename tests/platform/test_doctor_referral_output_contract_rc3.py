...
def a900043_request():
    return {
        ...
        "case_id":"CASE-20260808-A900043-001",
        "article_id":"A900043",
        "site_id":"sample-site",
        "request_mode":"DOCTOR_REFERRAL_TREATMENT",
        "article":{"url":"https://example.invalid/article-43"},
...
def test_a900043_routes_to_treatment_result():
    assert expected_output_contract(a900043_request()) == ("SIMS_WRITER_TREATMENT_RESULT_V1","1.0")


def test_a900043_feedback_v2_is_blocked():
    issues=validate_output_contract_for_request(a900043_request(), {"format":"SIMS_FEEDBACK_V2","contract_version":"4.2"})
    assert issues
    assert "SIMS_WRITER_TREATMENT_RESULT_V1" in issues[0]


def test_a900043_treatment_result_is_accepted():
    ...
    result=build_treatment_result_v1(request=a900043_request(),publication_result=publication,completed_at="2026-08-08T08:10:00+09:00",recommended_review_days=35,next_action="remeasure")
    assert result["format"]=="SIMS_WRITER_TREATMENT_RESULT_V1"
    assert result["case_id"]=="CASE-20260808-A900043-001"
    assert result["publication_result"]==publication
    assert validate_output_contract_for_request(a900043_request(),result)==[]
