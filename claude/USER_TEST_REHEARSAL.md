...
## Repository RuntimeでのUATセッション準備

開発者は実記事依頼を直接ばらばらに管理せず、Repository Runtimeの`--prepare-claude-uat-session`でセッションを作成します。生成された証拠テンプレートは未実測の`pending`であり、Claude Project上の確認後だけ更新します。

## 最終一括判定

実記事証拠と初心者セットアップ実測の記入後、Runtimeで次を実行します。

```bash
python -m runtime.sims_writer_runtime.cli --run-claude-uat-readiness <session-directory> --output <report-directory>
```
